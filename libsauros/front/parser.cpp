...
std::vector<token_c *> tokenize(size_t line_number, const std::string line,
  ...
  for (size_t idx = 0; idx < line.size(); idx++) {
    ...
    case '"': {
      ...
      tokens.push_back(new string_token_c({line_number, start},
    ...
    if (is_digit(line[idx]) || (line[idx] == '-' && is_digit(line[idx + 1]))) {
      ...
      if (std::regex_match(current_data, is_number)) {
        if (current_data.find('.') != std::string::npos) {
          ...
          tokens.push_back(new integer_token_c({line_number, start}, actual));
          continue;
        }
      } else {
...
void throw_no_list_error(token_c *current_token,
                         std::shared_ptr<std::string> origin) {
  throw parser_exception_c("Attempting to create object prior to list creation",
                           origin, (*current_token).location);
}
} // namespace
