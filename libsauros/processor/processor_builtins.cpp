...
#define SAUROS_PROCESSOR_CHECK_CELL_SIZE(cells__, size__, command__)           \
  if (cells__.size() != size__) {                                              \
    throw exceptions::runtime_c(                                               \
        format("`%` expects % parameters", command__, size__ - 1), cells[0]);  \
  }
...
void processor_c::populate_standard_builtins() {
  ...
  _builtins[BUILTIN_ASSERT] =
      std::make_shared<cell_c>([this](cells_t &cells, env_ptr env) -> cell_ptr {
        ...
        for (auto c = cells.begin() + 2; c != cells.end(); c++) {
          ...
          if (cell_type_e::STRING == result->type && result->data.s->empty()) {
  ...
  _builtins[BUILTIN_VAR] = std::make_shared<cell_c>(
      [this, expect_var_get_name](cells_t &cells, env_ptr env) -> cell_ptr {
        ...
        if (cells.size() < 2) {
          throw exceptions::runtime_c("Nothing given to var command", cells[0]);
        }
        ...
        if (cells.size() != 3) {
          throw exceptions::runtime_c("var command expects 2 parameters, but " +
  ...
  _builtins[BUILTIN_SET_AT] =
      std::make_shared<cell_c>([this](cells_t &cells, env_ptr env) -> cell_ptr {
        ...
        if (idx->type != cell_type_e::INTEGER) {
          throw exceptions::runtime_c("set_at requires index to be an integer",
                                      cells[1]);
        }
  ...
  _builtins[BUILTIN_OR] =
      std::make_shared<cell_c>([this](cells_t &cells, env_ptr env) -> cell_ptr {
        ...
        constexpr auto fn = [](double lhs, double rhs) -> double {
          bool result =
              (static_cast<bool>(lhs) || static_cast<bool>(rhs)) ? true : false;
          return result;
        };
        PERFORM_ARITH("%", cells, fn, env, false);
      });
  ...
  auto conversion_fn =
    ...
    try {
      ...
      throw exceptions::runtime_c("Item caused out of range exception", target);
    }
  };
  ...
  _builtins[BUILTIN_THREAD] = std::make_shared<cell_c>(
      [this, expect_var_get_name](cells_t &cells, env_ptr env) -> cell_ptr {
        ...
        // Setup thread cell
        auto thread_cell = std::make_shared<thread_cell_c>(cells[0]->location);
        ...
        thread_cell->thread = std::thread(
            &processor_c::process_cell, thread_cell->processor, cells[1], env);
...
#endif
        SAUROS_PROCESSOR_CHECK_CELL_SIZE(cells, 1, "chan");
